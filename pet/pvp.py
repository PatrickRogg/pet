...
class CompanyClassificationPVP(PVP):
    ...
    def get_parts(self, example: InputExample) -> FilledPattern:
        ...
        if self.pattern_id == 0:
            return [self.mask, self.mask, self.mask, self.mask, self.mask, self.mask, self.mask, ':', text], []
        elif self.pattern_id == 1:
            return [self.mask, self.mask, self.mask, self.mask, self.mask, self.mask, self.mask, 'Category:', text], []
        elif self.pattern_id == 2:
            return ['[ Category:', self.mask, self.mask, self.mask, self.mask, self.mask, self.mask, self.mask, ']', text], []
        elif self.pattern_id == 3:
            return [self.mask, self.mask, self.mask, self.mask, self.mask, self.mask, self.mask, '-', text], []
        else:
            raise ValueError("No pattern implemented for id {}".format(self.pattern_id))
